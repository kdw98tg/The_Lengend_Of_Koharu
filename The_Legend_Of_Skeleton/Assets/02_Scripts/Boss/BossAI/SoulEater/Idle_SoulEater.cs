...
public class Idle_SoulEater : BossState
{
    ...
    public override void CheckState()
    {
        ...
        if (distance <= (m_Boss as Boss_SoulEater).IdleRange)//플레이어랑 거리가 30미만이면 Trace
        {
    ...
    {
        float distance = Vector3.Distance(m_Boss.PlayerTr.position, m_Boss.transform.position);
        if (distance >= (m_Boss as Boss_SoulEater).IdleRange && (m_Boss as Boss_SoulEater).CurrentHp <= (m_Boss as Boss_SoulEater).MaxHp)//전투 중에 보스지역 벗어나면 보스 제자리가고 체력참
        {
